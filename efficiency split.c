...
const double n_clad = n_core - 0.25;

const unsigned int N_sim = 1 << 10;
...
//const double width_in = 0.4; // microns
const double width_MMI = 5.0; // microns
...
int slove_waveguide(double *, double **, double ***, char);
...
//	field_to_csv(num_modes_in, field_in);
	*/
	...
	double *dker_eff_MMI;
	double **field_MMI;
	int num_modes_MMI = slove_waveguide(dker_MMI, &dker_eff_MMI, &field_MMI, 0);
...
//	field_to_csv(num_modes_MMI, field_MMI);
	...
	FILE *file = fopen("transition loss dn=.csv", "wt");
	for (N_in = 2; N_in <= N_MMI/2; N_in+=2)
	{
		uwidth_in  = (double) N_in  / (double) N_MMI;
		width_in = width_MMI * uwidth_in / uwidth_MMI;
		kwidth_in = wavenumber * width_in;
		printf("width_in = %f microns\n", width_in);
		
		symm_slab_dker(dker_in, N_in);
		double *dker_eff_in;
		double **field_in;
		int num_modes_in = slove_waveguide(dker_in, &dker_eff_in, &field_in, 1);
		printf("num_modes_in = %d\n", num_modes_in);
		printf("n_eff_in\n");
		for (int j = 0; j < num_modes_in; j++) printf("%f\n", sqrt(dker_eff_in[j]) / kx_inc);
		
		fprintf(file, "%e,%d,%e\n", width_in, num_modes_in, 1.0 - simulate_MMI(num_modes_MMI, dker_eff_MMI, field_MMI, field_in[0]));
...
int slove_waveguide(double *dker, double **dker_eff, double ***field, char single)
{
...
//	MMI_picture(num_modes, n_eff_MMI, field_scaled);
	...
	for (int k = N_MMI; k < 2 * N_MMI; k++)
	{
		...
		phase = 0.0;
		for (int i = 0; i < N_sim; i++) phase += field_start[i] * field[i];
		objective = cabs(phase);
	...
	FILE *file;
	file = fopen("reimage length dn=.csv", "at");
	fprintf(file, "%e\n", klength_reimage / wavenumber);
	fclose(file);
	
	field_MMI_depth(klength_reimage, num_modes, n_eff_MMI, field_scaled, field);
	phase = 0.0;
	for (int i = 0; i < N_sim; i++) phase += field_in[i] * field[i];
	double reimage_efficiency = phase * conj(phase);
	printf("re-image efficiency %f\n", reimage_efficiency);
	
	file = fopen("reimage loss dn=.csv", "at");
	fprintf(file, "%e\n", 1.0 - reimage_efficiency);
	fclose(file);
	
	field_MMI_depth(klength_reimage / 2.0, num_modes, n_eff_MMI, field_scaled, field);
	...
	file = fopen("split width dn=.csv", "at");
	fprintf(file, "%e\n", (kwidth_sim - 2 * peak_index * kx_inc) / wavenumber);
	fclose(file);
	
	phase = 0.0;
	for (int i = 0; i < N_sim/2; i++) phase += field[i] * field_in[i + N_sim/2 - peak_index];
	double split_efficiency = phase * conj(phase);
/*	double split_efficiency = 0.0;
	for (int i = 0; i < N_sim/2; i++) split_efficiency += cabs(field[i] * field_in[i + N_sim/2 - peak_index]);
	split_efficiency *= split_efficiency;*/
	printf("split efficiency %f\n", split_efficiency);
	
	file = fopen("split loss dn=.csv", "at");
	fprintf(file, "%e\n", 0.5 - split_efficiency);
	fclose(file);
