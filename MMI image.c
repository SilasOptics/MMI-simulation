...
//	const unsigned int N_core_in = N >> 4;
	...
	const double kx_inc = kL / (double) N; // step size
	
	const double kdepth_repeat = ncore * kW_MMI * kW_MMI / 2.0 / M_PI;
//	printf("analytic repeat length %f\n", kdepth_repeat / k0);
	
	const double dker_core = kx_inc * kx_inc * ncore * ncore; // relative permittivity normalised by the step size
	const double dker_clad = kx_inc * kx_inc * nclad * nclad;
