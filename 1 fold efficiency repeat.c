...
//	field_to_csv(num_modes_MMI, field_MMI);
	...
	FILE *file = fopen("reimage loss dn=.csv", "wt");
...
//	MMI_picture(num_modes, n_eff_MMI, field_scaled);
	...
	for (int k = N_MMI; k < 2 * N_MMI; k++)
	{
		...
		for (int i = 0; i < N_sim; i++) objective += fabs(field_in[i]) * cabs(field[i]);
	...
	FILE *file = fopen("reimage length dn=.csv", "at");
	fprintf(file, "%e\n", klength_reimage / wavenumber);
	fclose(file);
	
	field_MMI_depth(klength_reimage, num_modes, n_eff_MMI, field_scaled, field);
	double reimage_efficiency;
	for (int i = 0; i < N_sim; i++) reimage_efficiency += fabs(field_in[i]) * cabs(field[i]);
